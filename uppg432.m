...
sigma = 0.03;
input = input + sigma*randn(size(input)); % add noise
output = x(t+5);
...
%     drawnow
    ...
    v_error(k) = mean((norm(b3-t_valid)).^2);
    ...
    if (k > T && v_error(k)>v_error(k-1) && v_error(k)>v_error(k-2) && v_error(k)>v_error(k-3) && v_error(k)>v_error(k-4) && v_error(k)>v_error(k-5))
...
figure()
[~,iMin] = min(v_error);
minVec = NaN(length(v_error),1);
minVec(iMin) = v_error(iMin);
plot(v_error)
hold on
plot(minVec, '*')
hold off
