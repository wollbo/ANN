...
for i = 1:epochs
   ...
   for j = randperm(349)
        ...
        for k = -n_func:n_func
            update_index = [update_index find((abs((1:100)-(index + k*10)) <= n_func).*(1:100))];
        end
...
for i = 0:7
    ...
    uni = unique(ind);
    cnt = histc(ind,uni);
    scatter(coord(uni,1),coord(uni,2),cnt*100,'markerfacecolor',[color_code(1+i,:)],'markeredgecolor',[color_code(1+i,:)],'LineWidth',2)
    hold on
    axis([-0.1 1 -0.1 1])
